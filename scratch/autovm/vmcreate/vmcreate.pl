...
#
BEGIN { unshift(@INC,"/usr/local/lib/vmware-vcli/apps") if -d "/usr/local/lib/vmware-vcli/apps";}
...
sub create_vms {
   ...
   foreach (@vms) {
      ...
      my $nic_poweron = 1;
      my $resource_pool = 'Deployment_Lab';
      ...
      if ($_->findvalue('Resource-Pool')) {
         $resource_pool = $_->findvalue('Resource-Pool');
      }
...
sub create_vm {
   ...
   eval {
      $vm_folder_view->CreateVM(config => $vm_config_spec, pool => $respool_handle);
